...
export class ObservabilityService {
  ...
  endTrace(traceId: string, status: 'success' | 'error' = 'success', error?: Error): void {
    ...
    if (trace.duration > 5000) { // 5 seconds
      this.logger.warn(`Slow operation detected: ${trace.operationName} took ${trace.duration}ms`);
    }
  ...
  getPerformanceSummary(timeRange?: { start: Date; end: Date }): {
    ...
    const totalRequests = this.getAggregatedMetrics('operation_count', 'sum', timeRange);
  ...
  getActiveTraces(): PerformanceTrace[] {
    return Array.from(this.traces.values()).filter(t => t.status === 'pending');
  }
  ...
  registerHealthCheck(
    name: string,
    checkFunction: () => Promise<{ status: HealthCheck['status']; error?: string; metadata?: Record<string, unknown> }>
  ): void {
  ...
  exportPrometheusMetrics(): string {
    ...
    for (const [name, metrics] of this.metrics.entries()) {
      ...
      // Add help and type
      lines.push(`# HELP ${name} Generated metric from Zyra AI service`);
  ...
  private checkAlerts(metric: Metric): void {
    ...
    // Memory usage alert
    if (metric.name === 'memory_heap_used' && metric.value > 1024 * 1024 * 1024) { // 1GB
      this.createAlert(
        'high_memory_usage',
        'warning',
