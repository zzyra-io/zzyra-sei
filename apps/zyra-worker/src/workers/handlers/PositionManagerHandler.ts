...
export class PositionManagerHandler extends AbstractBlockHandler {
  ...
  async execute(node: any, ctx: BlockExecutionContext): Promise<any> {
    ...
    try {
      ...
      switch (validatedConfig.action) {
        case 'create':
          result = await this.createPosition(protocolProvider, validatedConfig, wallet, executionId, nodeId);
          break;
        case 'adjust':
          result = await this.adjustPosition(protocolProvider, validatedConfig, wallet, executionId, nodeId);
          break;
        case 'close':
          result = await this.closePosition(protocolProvider, validatedConfig, wallet, executionId, nodeId);
          break;
        case 'monitor':
          result = await this.monitorPosition(protocolProvider, validatedConfig, wallet, executionId, nodeId);
          break;
        default:
   ...
   */
  private validateConfig(config: Record<string, any>): PositionManagerConfig {
    try {
      ...
      switch (parsedConfig.action) {
        case 'create':
          if (!parsedConfig.amountA || !parsedConfig.amountB) {
            throw new Error('Both amountA and amountB are required for creating a position');
          }
        ...
        case 'close':
          if (!parsedConfig.positionId) {
            throw new Error('Position ID is required for adjusting or closing a position');
          }
          break;
        case 'monitor':
          if (!parsedConfig.positionId) {
            throw new Error('Position ID is required for monitoring a position');
          }
          break;
      }
      ...
      throw new Error(`Invalid position manager configuration: ${error?.message || 'Unknown validation error'}`);
    }
  }
  ...
  private async createPosition(
    ...
    try {
      ...
      const balanceABefore = await this.checkAssetBalance(wallet, config.tokenA);
      const balanceBBefore = await this.checkAssetBalance(wallet, config.tokenB);
  ...
  private async adjustPosition(
    ...
    try {
      ...
      const position = await this.getPositionDetails(protocolProvider, config.positionId!);
      ...
      const isAdding = config.amountA !== undefined || config.amountB !== undefined;
      
      let adjustResult;
      if (isAdding) {
        // Adding liquidity
        const amountA = config.amountA ? 
          ethers.parseUnits(config.amountA, this.getTokenDecimals(config.tokenA)) : 
          ethers.ZeroAddress;
        
        const amountB = config.amountB ? 
          ethers.parseUnits(config.amountB, this.getTokenDecimals(config.tokenB)) : 
          ethers.ZeroAddress;
      ...
      const updatedPosition = await this.getPositionDetails(protocolProvider, config.positionId!);
      ...
      throw new Error(`Failed to adjust position: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  private async closePosition(
    ...
    try {
      ...
      return {
        ...
        tokenA: config.tokenA,
        tokenB: config.tokenB,
        amountAReceived: (parseFloat(balanceAAfter) - parseFloat(balanceABefore)).toString(),
        amountBReceived: (parseFloat(balanceBAfter) - parseFloat(balanceBBefore)).toString(),
        balanceABefore,
        balanceAAfter,
        balanceBBefore,
        balanceBAfter,
      ...
      throw new Error(`Failed to close position: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  private async monitorPosition(
    ...
    try {
      ...
      // Get current price
      const currentPrice = await protocolProvider.getPrice(config.tokenA, config.tokenB);
      
      // Calculate position value
      const positionValue = await protocolProvider.getPositionValue(config.positionId!);
      
      // Calculate uncollected fees
      const uncollectedFees = await protocolProvider.getUncollectedFees(config.positionId!);
      ...
      throw new Error(`Failed to monitor position: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  private async getPositionDetails(
    ...
    try {
      ...
      throw new Error(`Failed to get position details: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  private async storePosition(position: any): Promise<void> {
    try {
      ...
      if (error) {
        this.logger.error(`Failed to store position: ${String(error)}`);
      }
    } catch (error: any) {
      this.logger.error(`Error storing position: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  private async updatePositionStatus(positionId: string, status: string): Promise<void> {
    try {
      ...
      if (error) {
        this.logger.error(`Failed to update position status: ${String(error)}`);
      }
    } catch (error: any) {
      this.logger.error(`Error updating position status: ${error?.message || 'Unknown error'}`);
    }
  }
   ...
   */
  private parsePositionIdFromReceipt(receipt: ethers.TransactionReceipt): string {
    try {
      ...
      this.logger.warn(`Failed to parse position ID from receipt: ${error?.message || 'Unknown error'}`);
