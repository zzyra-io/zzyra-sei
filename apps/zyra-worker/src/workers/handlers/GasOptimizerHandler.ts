...
const GasOptimizerConfigSchema = z.object({
  ...
  gasStrategy: z
    .enum(['aggressive', 'balanced', 'economic'])
    .default('balanced'),
...
export class GasOptimizerHandler implements BlockHandler {
  ...
  private validateConfig(config: Record<string, any>): GasOptimizerConfig {
    try {
      ...
      throw new Error(
        `Invalid gas optimizer configuration: ${error?.message || 'Unknown validation error'}`,
      );
    }
  }
  ...
  private initializeProviders(): void {
    ...
    this.logger.log(
      `Initialized providers for networks: ${Object.keys(this.providers).join(', ')}`,
    );
  }
  ...
  private async getCurrentGasPrices(
    ...
    try {
      ...
      throw new Error(
        `Failed to get current gas prices: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  private calculateOptimalGasPrice(
    ...
    try {
      ...
      throw new Error(
        `Failed to calculate optimal gas price: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  private async estimateWaitTime(
    ...
    try {
      ...
      const avgBlockGasPrice =
        blockGasPrices.reduce((sum, price) => sum + price, 0) /
        blockGasPrices.length;
      ...
      if (useEIP1559) {
        ...
        if (
          userMaxFeePerGas > avgBlockGasPrice * 1.5 &&
          userPriorityFee > 1.5
        ) {
          waitTimeMinutes = 0.5; // Very fast: next block
        } else if (
          userMaxFeePerGas > avgBlockGasPrice * 1.2 &&
          userPriorityFee > 1
        ) {
          waitTimeMinutes = 2; // Fast: 2-3 blocks
        } else if (
          userMaxFeePerGas >= avgBlockGasPrice &&
          userPriorityFee > 0.5
        ) {
      ...
      this.logger.warn(
        `Failed to estimate wait time: ${error?.message || 'Unknown error'}`,
      );
