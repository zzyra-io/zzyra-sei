...
const YieldStrategyConfigSchema = z.object({
  ...
  minApy: z.number().min(0).default(0), // Minimum APY to consider
  maxRisk: z.number().min(0).max(10).default(10), // Maximum risk score (1-10 scale)
  lockupPeriodMax: z.number().optional(), // Maximum lockup period in days
  protocolAllowList: z.array(z.string()).optional(), // Only consider these protocols
  protocolDenyList: z.array(z.string()).optional(), // Exclude these protocols
});
...
export class YieldStrategyHandler implements BlockHandler {
  ...
  async execute(node: any, ctx: BlockExecutionContext): Promise<any> {
    ...
    try {
      ...
      const filteredOptions = this.filterYieldOptions(
        yieldOptions,
        validatedConfig,
      );
      ...
      const rankedOptions = this.rankYieldOptions(
        filteredOptions,
        validatedConfig,
      );
  ...
  private validateConfig(config: Record<string, any>): YieldStrategyConfig {
    try {
      ...
      throw new Error(
        `Invalid yield strategy configuration: ${error?.message || 'Unknown validation error'}`,
      );
    }
  }
  ...
  private async getYieldOptions(asset: string): Promise<ProtocolYieldOption[]> {
    try {
      ...
      const protocols =
        await this.protocolService.getProtocolsWithYieldForAsset(asset);
      ...
      throw new Error(
        `Failed to get yield options: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  private filterYieldOptions(
    ...
    return options.filter((option) => {
      ...
      if (
        config.lockupPeriodMax !== undefined &&
        option.lockupPeriod > config.lockupPeriodMax
      ) {
        return false;
      }
    ...
    switch (riskTolerance) {
      case 'low':
        ...
        if (rankedOptions.length >= 3) {
          allocations = [
            {
              protocol: rankedOptions[0].protocol,
              percentage: 50,
              amount: totalAmount * 0.5,
            },
            {
              protocol: rankedOptions[1].protocol,
              percentage: 30,
              amount: totalAmount * 0.3,
            },
            {
              protocol: rankedOptions[2].protocol,
              percentage: 20,
      ...
      case 'medium':
        ...
        if (rankedOptions.length >= 4) {
          allocations = [
            ...
            {
              protocol: rankedOptions[3].protocol,
              percentage: 15,
  ...
  /**
   * Starts execution tracking for a block
   */
  ...
  /**
   * Completes execution tracking for a block
   */
  ...
  /**
   * Tracks a log entry for an execution
   */
