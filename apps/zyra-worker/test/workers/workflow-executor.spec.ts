...
describe('WorkflowExecutor', () => {
  ...
  describe('buildNodeDependencyMap', () => {
    it('should build a dependency map based on edges', () => {
      ...
      const dependencyMap = workflowExecutor.buildNodeDependencyMap(
        sampleNodes,
        sampleEdges,
      );
  ...
  describe('getRelevantOutputs', () => {
    it('should get outputs relevant to a node based on its dependencies', () => {
      ...
      // @ts-ignore - private method
      const relevantOutputs = workflowExecutor.getRelevantOutputs(
        'node2',
        dependencyMap,
        allOutputs,
      );
  ...
  describe('Data Flow with Edge Configuration', () => {
    it('should pass data between blocks with field mapping', async () => {
      const nodes = [
        {
          id: 'node1',
          data: {
      ...
      // Mock the node executor to return test data
      const mockNodeExecutor = {
        executeNode: jest
          .fn()
      ...
      const workflowExecutor = new WorkflowExecutor(
        ...
        mockNodeExecutor as any,
        {} as any, // executionLogger
        {} as any, // notificationService
        {} as any, // executionMonitorService
        {} as any, // multiLevelCircuitBreaker
        {} as any, // dataTransformationService
        {} as any, // dataStateService
        {} as any, // parallelExecutionService
        {} as any, // blockchainDataSyncService
      );
      ...
      // Verify that the second node received the mapped data
      const secondCall = mockNodeExecutor.executeNode.mock.calls[1];
      const secondNodeContext = secondCall[1]; // BlockExecutionContext

      expect(secondNodeContext.previousOutputs).toBeDefined();
      expect(secondNodeContext.previousOutputs.node1).toBeDefined();
      expect(secondNodeContext.previousOutputs.node1.price).toBe(50000);
      expect(secondNodeContext.previousOutputs.node1.status).toBe('success');
    });

    it('should handle conditional execution based on edge conditions', async () => {
      const nodes = [
        {
          id: 'node1',
          data: {
