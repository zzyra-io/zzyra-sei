...
describe('NodeExecutor', () => {
  ...
  describe('executeNode', () => {
    it('should execute a node successfully', async () => {
      ...
      const mockHandler = require('../../src/workers/handlers/BlockHandlerRegistry').BlockHandlerRegistry().getAllHandlers()[BlockType.EMAIL];
      mockHandler.execute.mockResolvedValue({ success: true, data: { result: 'email-sent' } });
      ...
      expect(mockSupabase.client.from).toHaveBeenCalledWith('execution_node_status');
    ...
    it('should retry failed node executions', async () => {
      ...
      mockHandler.execute
        ...
        .mockResolvedValueOnce({ success: true, data: { result: 'email-sent' } });
    ...
    it('should handle node execution timeout', async () => {
      ...
      const mockHandler = require('../../src/workers/handlers/BlockHandlerRegistry').BlockHandlerRegistry().getAllHandlers()[BlockType.EMAIL];
      mockHandler.execute.mockImplementation(() => new Promise((resolve) => {
  ...
  describe('prepareBlockData', () => {
    it('should prepare data for AI_BLOCKCHAIN blocks', () => {
      ...
      const result = (nodeExecutor as any).prepareBlockData(node, BlockType.AI_BLOCKCHAIN);
    ...
    it('should prepare data for EMAIL blocks', () => {
      ...
      // Call prepareBlockData
      const result = (nodeExecutor as any).prepareBlockData(node, BlockType.EMAIL);
    ...
    it('should prepare data for CUSTOM blocks', () => {
      ...
      // Call prepareBlockData
      const result = (nodeExecutor as any).prepareBlockData(node, BlockType.CUSTOM);
  ...
  describe('getOrCreateNodeExecution', () => {
    ...
    it('should create a new node execution if one does not exist', async () => {
      ...
      const mockNewNodeExecution = createMockNodeExecution({ node_id: 'node1' });
    ...
    it('should handle errors when creating a node execution', async () => {
      ...
      mockSupabase.setResponse({ data: null, error: { message: 'Failed to create node execution' } });
