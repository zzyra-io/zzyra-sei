...
import { NotificationService, NotificationType } from '../../src/services/notification.service';
    ...
    it('should send a workflow_completed notification', async () => {
      ...
      mockSupabase.setResponse({ 
        data: { 
          ...
          email_template: 'Your workflow {{workflow_name}} (ID: {{workflow_id}}) has completed successfully.',
      ...
      expect(mockSupabase.client.from).toHaveBeenCalledWith('notification_templates');
      expect(mockSupabase.mocks.select).toHaveBeenCalled();
      expect(mockSupabase.mocks.eq).toHaveBeenCalledWith('id', 'workflow_completed');
      ...
      expect(transport.sendMail).toHaveBeenCalledWith(expect.objectContaining({
        ...
        html: expect.stringContaining('Your workflow Test Workflow (ID: test-workflow-id) has completed successfully.'),
      }));
      
      // Verify that a notification log was created
      expect(mockSupabase.client.from).toHaveBeenCalledWith('notification_logs');
      expect(mockSupabase.mocks.insert).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should send a workflow_failed notification', async () => {
      ...
      mockSupabase.setResponse({ 
        data: { 
          ...
          email_template: 'Your workflow {{workflow_name}} (ID: {{workflow_id}}) has failed: {{error}}',
      ...
      expect(transport.sendMail).toHaveBeenCalledWith(expect.objectContaining({
        ...
        html: expect.stringContaining('Your workflow Test Workflow (ID: test-workflow-id) has failed: Something went wrong'),
      }));
    ...
    it('should handle template rendering errors', async () => {
      ...
      mockSupabase.setResponse({ 
        data: { 
          ...
          email_template: 'Your workflow {{workflow_name} has completed successfully.', // Missing closing brace
