 ...
 */
export function createDeFiPortfolioTemplate(_userId: string): {
  nodes: FlowNode[];
  edges: FlowEdge[];
} {
    ...
    // Notification
    {
      ...
      data: {
        ...
        config: {
          ...
          message:
            "Current portfolio value: {{data.totalValue}} USD. {{#if data.needsRebalance}}Rebalancing executed.{{else}}No rebalancing needed.{{/if}}",
        },
        inputs: [],
        outputs: [],
      },
