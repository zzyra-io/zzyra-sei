...
export class OllamaProvider implements AIProvider {
  ...
  async generateFlow(prompt: string, userId: string) {
    try {
      ...
      try {
        ...
        if (!response.ok) {
          console.warn(
            `Ollama API returned status ${response.status}, falling back to mock data`
          );
          return this.generateMockFlow(prompt);
        }
        ...
        try {
          ...
          console.warn(
            "Could not parse valid flow data from Ollama response, falling back to mock data"
          );
          return this.generateMockFlow(prompt);
        } catch (parseError) {
  ...
  private generateMockFlow(prompt: string) {
    ...
    const hasWallet =
      lowercasePrompt.includes("wallet") || lowercasePrompt.includes("balance");
    const hasNotification =
      lowercasePrompt.includes("notif") || lowercasePrompt.includes("alert");
    const hasTransaction =
      lowercasePrompt.includes("transaction") ||
      lowercasePrompt.includes("transfer") ||
      lowercasePrompt.includes("send");
    const hasToken =
      lowercasePrompt.includes("token") ||
      lowercasePrompt.includes("coin") ||
      lowercasePrompt.includes("eth");
    const hasCondition =
      lowercasePrompt.includes("if") ||
      lowercasePrompt.includes("when") ||
      lowercasePrompt.includes("condition");
