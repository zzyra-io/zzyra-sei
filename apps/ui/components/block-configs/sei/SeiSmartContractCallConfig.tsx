...
import { z } from 'zod';
import { seiSmartContractCallSchema, type SeiSmartContractCallConfig } from '@zyra/types';
...
export default function SeiSmartContractCallConfig({
  ...
  const addFunctionParam = () => {
    const newParams = [...(typedConfig.functionParams || []), { name: '', type: 'string', value: '' }];
    updateConfig({ functionParams: newParams });
  };

  const removeFunctionParam = (index: number) => {
    const newParams = (typedConfig.functionParams || []).filter((_, i) => i !== index);
    updateConfig({ functionParams: newParams });
  };
  ...
  return (
    <div className="space-y-6">
      <Alert>
        ...
        <AlertDescription>
          Smart contract calls can execute transactions on the blockchain and may involve costs. 
          Always test on testnet first and verify contract addresses carefully.
        </AlertDescription>
      </Alert>
        ...
        <TabsContent value="config" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Specify the smart contract to interact with</CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => updateConfig({ network: value as any })}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  {validationErrors.contractAddress && (
                    <p className="text-sm text-red-500 mt-1">{validationErrors.contractAddress}</p>
                  )}
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Specify the function to call and its parameters</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                {validationErrors.functionName && (
                  <p className="text-sm text-red-500 mt-1">{validationErrors.functionName}</p>
                )}
              </div>
              ...
              <div>
                <Label htmlFor="functionSignature">Function Signature (Alternative to ABI)</Label>
                <Input
                  id="functionSignature"
                  placeholder="transfer(address,uint256)"
                  value={typedConfig.functionSignature || ''}
                  onChange={(e) => updateConfig({ functionSignature: e.target.value })}
                />
              </div>
                ...
                <div className="space-y-2">
                  {(typedConfig.functionParams || []).map((param, index) => (
                    <div key={index} className="grid grid-cols-4 gap-2">
                      ...
                      <Select
                        value={param.type}
                        onValueChange={(value) => updateFunctionParam(index, 'type', value)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                      ...
                      <Input
                        placeholder="Value"
                        value={param.value}
                        onChange={(e) => updateFunctionParam(index, 'value', e.target.value)}
                      />
                      <Button
                        variant="outline"
                        size="sm"
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure wallet for transaction signing</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => updateConfig({
                    walletConfig: { ...typedConfig.walletConfig, type: value as any }
                  })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  ...
                  <SelectContent>
                    <SelectItem value="magic_wallet">Magic Wallet (Recommended)</SelectItem>
              ...
              {typedConfig.walletConfig.type !== 'magic_wallet' && (
                <Alert>
                  ...
                  <AlertDescription>
                    Private keys and mnemonics are encrypted and stored securely. 
                    Consider using Magic Wallet for enhanced security.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure gas, value, and confirmation settings</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <Label htmlFor="confirmationTimeout">Confirmation Timeout (ms)</Label>
                  <Input
                    id="confirmationTimeout"
                    type="number"
                    min="1000"
                    value={typedConfig.confirmationTimeout}
                    onChange={(e) => updateConfig({ confirmationTimeout: parseInt(e.target.value) || 60000 })}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => updateConfig({ waitForConfirmation: !!checked })}
                />
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    <Label htmlFor="gasPrice" className="text-sm">Gas Price</Label>
                    <Input
                      id="gasPrice"
                      type="number"
                      placeholder="Auto"
                  ...
                  <div className="flex items-end">
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="estimateGas" className="text-sm">Auto Estimate</Label>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <TabsContent value="inputs" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>This block accepts dynamic parameters and gas overrides</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="outputs" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Transaction result with contract execution details</CardDescription>
            </CardHeader>
            <CardContent>
              <pre className="bg-gray-100 p-4 rounded text-sm">
{`{
        ...
        <TabsContent value="execution" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Monitor contract execution and test configuration</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {executionData?.error && (
                <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded">
                  ...
                  <div>
                    <h4 className="font-medium text-red-800">Contract Execution Error</h4>
              ...
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  Testing will execute a real transaction on the selected network. 
                  Make sure you have sufficient funds and have verified the contract details.
                </AlertDescription>
              </Alert>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-gray-600">{executionData?.startTime || 'Not executed'}</p>
                </div>
                <div>
                  <span className="font-medium">Execution Time:</span>
                  <p className="text-gray-600">
                    {executionData?.duration ? `${executionData.duration}ms` : 'N/A'}
                  </p>
                </div>
              </div>
            </CardContent>
