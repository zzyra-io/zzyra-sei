...
import { submitFeedback } from "@/lib/api/enhanced-workflow-generation";
...
export const FeedbackCollector = React.memo<FeedbackCollectorProps>(({
  ...
  const handleQuickFeedback = (positive: boolean) => {
    ...
    handleSubmit(positive ? 5 : 2, positive ? "Good experience" : "Needs improvement");
  };

  const handleSubmit = async (submitRating?: number, submitFeedback?: string) => {
    const finalRating = submitRating ?? rating;
    const finalFeedback = submitFeedback ?? feedback;
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Select value={selectedType} onValueChange={setSelectedType}>
            ...
            <SelectContent>
              ...
              <SelectItem value="block_generation">Block Generation</SelectItem>
        ...
        <div>
          <Label htmlFor="feedback-text">
            Feedback {rating > 0 && rating <= 2 && <span className="text-red-500">*</span>}
          </Label>
          <Textarea
            id="feedback-text"
            placeholder={
            ...
            <div className="mt-2">
              <RadioGroup value={executionResult} disabled>
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="success" className="text-sm">Success</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="partial" id="partial" />
                  <Label htmlFor="partial" className="text-sm">Partial Success</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="failure" id="failure" />
                  <Label htmlFor="failure" className="text-sm">Failure</Label>
                </div>
              </RadioGroup>
            </div>
          </div>
