...
export function RealtimeExecutionMonitor({
  ...
  const [nodeUpdates, setNodeUpdates] = useState<
    Map<string, NodeExecutionUpdate>
  >(new Map());
  ...
  useEffect(() => {
    ...
    socket.on("node_execution_update", (update: NodeExecutionUpdate) => {
      ...
      // Update current node in execution status with enhanced tracking
      if (update.status === "running") {
            ...
            ? {
                ...
                progress: {
                  ...
                  currentNode: {
                    ...
                    status: update.status as "running" | "completed" | "failed",
        ...
        // Log node start with animation
        console.log(
          `🚀 Node ${update.nodeLabel || update.nodeId} started execution`
        );
      } else if (update.status === "completed") {
        // Log successful completion with output summary
        const outputSize = update.output
        ...
        console.log(
          `❌ Node ${update.nodeLabel || update.nodeId} failed: ${update.error}`
        );
      }
    });
  ...
  const filteredLogs =
    executionStatus?.logs.filter(
      (log) => selectedLogLevel === "all" || log.level === selectedLogLevel
    ) || [];
      ...
      <Card className='w-full'>
        <CardContent className='p-6 text-center'>
          ...
          <p className='text-sm text-muted-foreground mt-2'>
            Start a workflow execution to see real-time monitoring
          </p>
        </CardContent>
      </Card>
    );
          ...
          {executionStatus && (
            <div className='space-y-2'>
              ...
              <div className='flex items-center justify-between text-sm text-gray-600'>
                <span>
                  {executionStatus.progress.completedNodes} /{" "}
                  {executionStatus.progress.totalNodes} nodes completed
                </span>
                <span>
                ...
                <AlertDescription>
                  <div className='flex items-center justify-between'>
                    <span>
                      Executing:{" "}
                      <strong>
                        {executionStatus.progress.currentNode.label}
                      </strong>
                    </span>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='flex items-center space-x-2'>
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedLogLevel(e.target.value as any)
                          }
                  ...
                  <CardContent className='space-y-4'>
                    <div className='grid grid-cols-2 gap-4 text-sm'>
                      ...
                      <div className='flex items-center space-x-2'>
                        ...
                        <span>
                          Duration: {Math.round(metrics.totalDuration)}ms
                        </span>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <ScrollArea className='h-32'>
                          <div className='space-y-1 text-xs'>
                            {Object.entries(metrics.nodeMetrics).map(
                              ([nodeId, nodeMetric]) => (
                                <div
                                  key={nodeId}
                                  ...
                                  <div className='flex space-x-2 text-gray-600'>
                                    ...
                                    <span>
                                      {Math.round(nodeMetric.outputSize / 1024)}
                                      KB
                                    </span>
                                  </div>
                                </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2'>
                    {Array.from(nodeUpdates.entries()).map(
                      ([nodeId, update]) => (
                        <Tooltip key={nodeId}>
                          <TooltipTrigger asChild>
                            <div
                              className={`p-2 rounded-md text-xs border-2 transition-all ${
                          ...
                          <TooltipContent>
                            <div className='text-xs'>
                              ...
                              {update.nodeType && (
                                <div>
                                  <strong>Type:</strong> {update.nodeType}
                                </div>
                              )}
                              {update.duration && (
                                <div>
