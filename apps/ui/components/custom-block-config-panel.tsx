...
import { type CustomBlockDefinition, type BlockParameter, DataType } from "@/types/custom-block"
...
export function CustomBlockConfigPanel({ blockId, config, onUpdate, onClose }: CustomBlockConfigPanelProps) {
      ...
      case DataType.NUMBER:
        return (
          <Input
            ...
            onChange={(e) => handleValueChange(input.name, Number.parseFloat(e.target.value) || 0)}
            placeholder={`Enter ${input.name}`}
          />
        )
      case DataType.BOOLEAN:
          ...
          <div>
            ...
            <Card>
              <CardHeader className="py-3 px-4">
                <CardTitle className="text-sm font-medium">Expected Outputs</CardTitle>
              </CardHeader>
