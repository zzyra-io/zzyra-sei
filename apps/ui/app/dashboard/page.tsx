...
export default function DashboardPage() {
  ...
  const filteredWorkflows = allWorkflows.filter((workflow) => {
    ...
    const matchesTab =
      activeTab === "all" ||
      (activeTab === "recent" &&
        new Date(workflow.updated_at || workflow.updatedAt).getTime() >
          Date.now() - 7 * 24 * 60 * 60 * 1000) ||
      (activeTab === "favorites" &&
        (workflow.isFavorite || workflow.is_favorite));
  ...
  const sortedWorkflows = [...filteredWorkflows].sort((a, b) => {
    switch (sortBy) {
      ...
      case "last-run":
        return (
          new Date(b.last_run || b.statistics?.lastExecutedAt || 0).getTime() -
          new Date(a.last_run || a.statistics?.lastExecutedAt || 0).getTime()
        );
      default:
        return 0;
    }
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align='end'>
                        <DropdownMenuItem
                          ...
                          className={sortBy === "newest" ? "bg-muted/50" : ""}>
                          Newest First
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => setSortBy("oldest")}
                          className={sortBy === "oldest" ? "bg-muted/50" : ""}>
                          Oldest First
                        </DropdownMenuItem>
                        <DropdownMenuItem
