  ...
  const handleRatingSubmit = async () => {
    ...
    try {
      ...
      setBlock({
        ...block,
        rating:
          (block.rating * block.usageCount + rating) / (block.usageCount + 1),
      });
    } catch (error) {
          ...
          <div className='md:w-2/3 space-y-4'>
            <Tabs defaultValue='details'>
              ...
              <TabsContent value='configuration' className='p-4'>
                ...
                <div className='bg-muted rounded-md p-3 overflow-auto max-h-[400px]'>
                  <pre className='text-xs'>
                    {JSON.stringify(block.blockData.configFields, null, 2)}
                  </pre>
                </div>
              </TabsContent>
              ...
              <TabsContent value='code' className='p-4'>
                ...
                <div className='bg-muted rounded-md p-3 overflow-auto max-h-[400px]'>
                  <pre className='text-xs'>
                    {block.blockData.code || "No code available"}
                  </pre>
                </div>
              </TabsContent>
            </Tabs>
                    ...
                    <div className='flex flex-wrap gap-1 mt-2'>
                      {block.tags.slice(0, 3).map((tag, index) => (
                        <Button key={index} variant='outline' size='sm' asChild>
                          <Link
                            href={`/blocks/shared?tag=${encodeURIComponent(
                              tag
                            )}`}>
                            {tag}
                          </Link>
