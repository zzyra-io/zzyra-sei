...
export class MagicService {
  constructor() {
    if (!MAGIC_SECRET_KEY) {
      console.warn('MAGIC_SECRET_KEY environment variable is not set, using mock implementation');
    }
   ...
   */
  async validateToken(didToken: string): Promise<{ email: string; issuer: string }> {
    try {
      ...
      try {
        ...
        console.warn('MagicService: Failed to parse token, using fallback validation');
      }
      ...
      console.log('MagicService: Token validation successful', { email, issuer });
