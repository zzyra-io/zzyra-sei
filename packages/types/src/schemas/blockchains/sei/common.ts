...
export const seiNetworkSchema = z.enum(['sei-mainnet', 'sei-testnet']).default('sei-testnet');

// Sei address validation schema
export const seiAddressSchema = z.string().regex(SEI_ADDRESS_REGEX, 'Invalid Sei address format');
...
export const formatSeiAmount = (amount: number, decimals: number = 6): string => {
  return (amount / Math.pow(10, decimals)).toFixed(decimals);
};

export const parseSeiAmount = (amount: string, decimals: number = 6): number => {
