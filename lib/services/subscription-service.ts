...
class SubscriptionService {
   ...
   */
  async getCurrentSubscription(): Promise<SubscriptionWithTier | null> {
   ...
   */
  async createCheckoutSession(tierId: string, interval: BillingInterval): Promise<{ url: string } | null> {
   ...
   */
  private async checkLimit(type: 'execution' | 'workflow'): Promise<boolean> {
    const subscription = await this.getCurrentSubscription()
  ...
  async canCreateWorkflow(): Promise<boolean> {
    return this.checkLimit('workflow')
  }
