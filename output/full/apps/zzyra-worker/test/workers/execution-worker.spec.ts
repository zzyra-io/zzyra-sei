...
describe('ExecutionWorker', () => {
  ...
  beforeEach(async () => {
    ...
    workflowService = moduleRef.get(
      WorkflowService,
    ) as jest.Mocked<WorkflowService>;
    workflowExecutor = moduleRef.get(
      WorkflowExecutor,
    ) as jest.Mocked<WorkflowExecutor>;
  ...
  describe('onModuleInit', () => {
    it('should create a channel and set up queue consumers', async () => {
      ...
      expect(mockAmqp.channelWrapper.on).toHaveBeenCalledWith(
        'error',
        expect.any(Function),
      );
      expect(mockAmqp.connection.on).toHaveBeenCalledWith(
        'error',
        expect.any(Function),
      );
    });
  ...
  describe('processJob', () => {
    ...
    it('should handle workflow execution failures', async () => {
      ...
      workflowExecutor.executeWorkflow.mockRejectedValue(
    ...
    it('should handle workflow not found', async () => {
      ...
      // @ts-ignore - private method
      await expect(
        executionWorker.processJob(mockJob, mockSupabase.client),
      ).rejects.toThrow();
  ...
  describe('fetchWorkflow', () => {
    ...
    it('should handle errors when fetching a workflow', async () => {
      ...
      // @ts-ignore - private method
      await expect(
        executionWorker.fetchWorkflow('test-workflow-id'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('fetchOrCreateProfile', () => {
    it('should fetch an existing profile', async () => {
      ...
      // @ts-ignore - private method
      const result = await executionWorker.fetchOrCreateProfile(
        'test-user-id',
        mockSupabase.client,
      );
    ...
    it('should handle errors when fetching a profile', async () => {
      ...
      // @ts-ignore - private method
      await expect(
        executionWorker.fetchOrCreateProfile(
          'test-user-id',
          mockSupabase.client,
        ),
      ).rejects.toThrow();
  ...
  describe('extractNodesAndEdges', () => {
    ...
    it('should throw an error if nodes and edges are missing', () => {
      ...
      // @ts-ignore - private method
      expect(() =>
        executionWorker.extractNodesAndEdges(mockWorkflow),
      ).toThrow();
    });
  });
});
