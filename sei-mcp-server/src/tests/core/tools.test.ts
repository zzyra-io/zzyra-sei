...
describe("EVM Tools", () => {
  ...
  beforeEach(async () => {
    ...
    (getSupportedNetworks as jest.Mock).mockReturnValue(["sei", "1328"]);
    (getPrivateKeyAsHex as jest.Mock).mockReturnValue(
    ...
    const mockWalletProvider = {
      ...
      getWalletClient: jest
        .fn()
        .mockResolvedValue({ account: { address: mockAddress } }),
    };
    (getWalletProvider as jest.Mock).mockReturnValue(mockWalletProvider);

    // Mock service functions
    (services.getAddressFromProvider as jest.Mock).mockResolvedValue(
      mockAddress
    );
    (services.getChainId as jest.Mock).mockResolvedValue(1 as never);
    (services.getBlockNumber as jest.Mock).mockResolvedValue(
      BigInt(12345678) as never
    );
    ...
    const helpersObj: ServiceHelpers = {
      formatJson: jest
        ...
        .mockImplementation((data: unknown) =>
          JSON.stringify(data)
        ) as unknown as (obj: unknown) => string,
      parseEther: jest.fn() as unknown as (
        ether: string,
        unit?: "wei" | "gwei"
      ) => bigint,
      validateAddress: jest.fn() as unknown as (
        address: string
      ) => `0x${string}`,
    };
  ...
  const checkToolExists = (toolName: string) => {
    ...
    if (!tool) {
      console.log(
        `Tool '${toolName}' not found. Available tools: ${Array.from(registeredTools.keys()).join(", ")}`
      );
    }
    return tool;
  };
  ...
  describe("Network Information Tools", () => {
    ...
    test("get_chain_info - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        { network: mockNetwork },
        services.getChainId as jest.Mock,
        mockError
      );
    ...
    test("get_chain_info - error with non-Error object", async () => {
      ...
      expect(response.content[0].text).toContain(
        "Error fetching chain info: This is a string error"
      );
    });
    ...
    test("get_chain_info - error with blockNumber", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching chain info: Test error"
      );
    });
    ...
    test("get_supported_networks - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        {},
        getSupportedNetworks as jest.Mock,
        mockError
      );

      verifyErrorResponse(
        response,
        "Error fetching supported networks: Test error"
      );
    });
    ...
    test("get_supported_networks - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching supported networks: This is a string error"
      );
    });
  });
  ...
  describe("Block Tools", () => {
    ...
    beforeEach(() => {
      (services.getBlockByNumber as jest.Mock).mockResolvedValue(
        mockBlock as never
      );
      (services.getLatestBlock as jest.Mock).mockResolvedValue(
        mockBlock as never
      );
    });
    ...
    test("get_block_by_number - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        { blockNumber, network: mockNetwork },
        services.getBlockByNumber as jest.Mock,
        mockError
      );

      verifyErrorResponse(
        response,
        `Error fetching block ${blockNumber}: Test error`
      );
    });
    ...
    test("get_block_by_number - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        `Error fetching block ${blockNumber}: This is a string error`
      );
    });
    ...
    test("get_latest_block - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        { network: mockNetwork },
        services.getLatestBlock as jest.Mock,
        mockError
      );
    ...
    test("get_latest_block - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching latest block: This is a string error"
      );
    });
  });
  ...
  describe("Balance Tools", () => {
    ...
    beforeEach(() => {
      (services.getBalance as jest.Mock).mockResolvedValue({
        wei: BigInt(100),
        sei: "0.0000000000000001",
      } as never);
      ...
      (services.getERC721Balance as jest.Mock).mockResolvedValue(
        BigInt(2) as never
      );
      (services.getERC1155Balance as jest.Mock).mockResolvedValue(
        BigInt(5) as never
      );
      (services.getERC20TokenInfo as jest.Mock).mockResolvedValue(
        mockTokenInfo as never
      );
      (services.getERC721TokenMetadata as jest.Mock).mockResolvedValue(
        mockNftInfo as never
      );
    });
    ...
    test("get_balance - success path", async () => {
      ...
      expect(services.getBalance).toHaveBeenCalledWith(
        mockAddress,
        mockNetwork
      );
    ...
    test("get_balance - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        { address: mockAddress, network: mockNetwork },
        services.getBalance as jest.Mock,
        mockError
      );
    ...
    test("get_balance - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching balance: This is a string error"
      );
    });
    ...
    test("get_erc20_balance - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        { tokenAddress, address, network: mockNetwork },
      ...
      verifyErrorResponse(
        response,
        "Error fetching ERC20 balance for 0x1234567890123456789012345678901234567890: Test error"
      );
    });
    ...
    test("get_erc20_balance - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        `Error fetching ERC20 balance for ${address}: This is a string error`
      );
    });
    ...
    test("get_token_balance - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching token balance: This is a string error"
      );
    });
    ...
    test("get_nft_balance - success path", async () => {
      ...
      expect(services.getERC721Balance).toHaveBeenCalledWith(
        mockTokenAddress,
        mockAddress,
        mockNetwork
      );
    ...
    test("get_nft_balance - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching NFT balance: This is a string error"
      );
    });
    ...
    test("get_erc1155_balance - success path", async () => {
      ...
      expect(services.getERC1155Balance).toHaveBeenCalledWith(
        mockTokenAddress,
        mockAddress,
        BigInt(mockTokenId),
        mockNetwork
    ...
    test("get_erc1155_balance - error path", async () => {
      ...
      verifyErrorResponse(
        response,
        "Error fetching ERC1155 token balance: Test error"
      );
    });
    ...
    test("get_erc1155_balance - error path with invalid token id", async () => {
      ...
      verifyErrorResponse(
        response,
        "Error fetching ERC1155 token balance: Cannot convert abc to a BigInt"
      );
    });
    ...
    test("get_erc1155_balance - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error fetching ERC1155 token balance: This is a string error"
      );
    });
  ...
  describe("Wallet Tools", () => {
    describe("get_address_from_private_key", () => {
      ...
      test("get_address_from_private_key - wallet not available", async () => {
        ...
        verifyErrorResponse(
          response,
          "Error: Wallet provider 'private-key' is not available"
        );
      });
      ...
      test("get_address_from_private_key - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          {},
          services.getAddressFromProvider as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error deriving address from private key: Test error"
        );
      });
      ...
      test("get_address_from_private_key - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error deriving address from private key: This is a string error"
        );
      });
    });
    ...
    test("should handle disabled wallet mode", () => {
      ...
      expect(disabledWalletTools.has("get_address_from_private_key")).toBe(
        false
      );
    ...
    describe("Transaction Tools", () => {
      const { mockHash, mockTransaction, mockTransactionReceipt } =
        setupTransactionMocks();

      beforeEach(() => {
        (services.getTransaction as jest.Mock).mockResolvedValue(
          mockTransaction as never
        );
        (services.getTransactionReceipt as jest.Mock).mockResolvedValue(
          mockTransactionReceipt as never
        );
        (services.estimateGas as jest.Mock).mockResolvedValue(
          BigInt(21000) as never
        );
      });
      ...
      test("get_transaction - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          { txHash, network: mockNetwork },
          services.getTransaction as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          `Error fetching transaction ${txHash}: Test error`
        );
      });
      ...
      test("get_transaction - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          `Error fetching transaction ${txHash}: This is a string error`
        );
      });
      ...
      test("get_transaction_receipt - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          { txHash, network: mockNetwork },
          services.getTransactionReceipt as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          `Error fetching transaction receipt ${txHash}: Test error`
        );
      });
      ...
      test("get_transaction_receipt - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          `Error fetching transaction receipt ${txHash}: This is a string error`
        );
      });
      ...
      test("estimate_gas - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          params,
          services.estimateGas as jest.Mock,
          mockError
        );
      ...
      test("estimate_gas - success path with default network", async () => {
        ...
        (services.helpers.parseEther as jest.Mock).mockReturnValueOnce(
      ...
      test("estimate_gas - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error estimating gas: This is a string error"
        );
      });
    ...
    describe("Transfer Tools", () => {
      ...
      beforeEach(() => {
        (services.transferSei as jest.Mock).mockResolvedValue(
          mockHash as never
        );
        ...
        (services.approveERC20 as jest.Mock).mockResolvedValue(
          mockHash as never
        );
        (services.transferERC721 as jest.Mock).mockResolvedValue(
          mockHash as never
        );
        (services.transferERC1155 as jest.Mock).mockResolvedValue(
          mockHash as never
        );
      });
      ...
      test("transfer_sei - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          transferParams,
          services.transferSei as jest.Mock,
          mockError
        );
      ...
      test("transfer_sei - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring Sei: This is a string error"
        );
      });
      ...
      test("transfer_sei - error with invalid amount", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring Sei: Invalid amount"
        );
      });
      ...
      test("approve_token_spending - success path with default network", async () => {
        ...
        (services.approveERC20 as jest.Mock).mockImplementationOnce(
          (tokenAddress, spenderAddress, amount, network) => {
            return Promise.resolve(mockHash);
          }
        );
      ...
      test("approve_token_spending - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error approving token spending: This is a string error"
        );
      });
      ...
      test("transfer_erc20 - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          transferParams,
          services.transferERC20 as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error transferring ERC20 tokens: Test error"
        );
      });
      ...
      test("transfer_erc20 - success path with default network", async () => {
        ...
        (services.transferERC20 as jest.Mock).mockImplementationOnce(
          (tokenAddress, toAddress, amount, network) => {
      ...
      test("transfer_erc20 - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring ERC20 tokens: This is a string error"
        );
      });
      ...
      test("transfer_nft - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          transferParams,
          services.transferERC721 as jest.Mock,
          mockError
        );
      ...
      test("transfer_nft - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring NFT: This is a string error"
        );
      });
      ...
      test("read_contract - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error reading contract: This is a string error"
        );
      });
      ...
      test("transfer_nft - success path with default network", async () => {
        ...
        (services.transferERC721 as jest.Mock).mockImplementationOnce(
          (tokenAddress, toAddress, tokenId, network) => {
            return Promise.resolve(mockHash);
          }
        );
      ...
      test("transfer_erc1155 - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          transferParams,
          services.transferERC1155 as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error transferring ERC1155 tokens: Test error"
        );
      });
      ...
      test("transfer_erc1155 - success path with default network", async () => {
        ...
        (services.transferERC1155 as jest.Mock).mockImplementationOnce(
          (tokenAddress, toAddress, tokenId, amount, network) => {
            return Promise.resolve(mockHash);
          }
        );
      ...
      test("transfer_erc1155 - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring ERC1155 tokens: This is a string error"
        );
      });
      ...
      test("transfer_token - error with non-numeric amount", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring tokens: Invalid amount"
        );
      });
      ...
      test("transfer_token - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring tokens: This is a string error"
        );
      });
      ...
      test("transfer_token - error with unsupported token type", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error transferring tokens: Unsupported token type"
        );
      });
    ...
    describe("Contract Tools", () => {
      beforeEach(() => {
        (services.readContract as jest.Mock).mockResolvedValue(
          "0x1234" as never
        );
        (services.writeContract as jest.Mock).mockResolvedValue(
          "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef" as never
        );
      ...
      test("read_contract - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          readParams,
          services.readContract as jest.Mock,
          mockError
        );
      ...
      test("write_contract - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          writeParams,
          services.writeContract as jest.Mock,
          mockError
        );
      ...
      test("write_contract - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error writing to contract: This is a string error"
        );
      });
      ...
      test("is_contract - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error checking if address is a contract: This is a string error"
        );
      });
      ...
      test("is_contract - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          params,
          services.isContract as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error checking if address is a contract: Test error"
        );
      });
    });
    ...
    describe("Token Information Tools", () => {
      ...
      test("get_token_info - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error fetching token info: This is a string error"
        );
      });
      ...
      test("get_nft_info - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error fetching NFT info: This is a string error"
        );
      });
      ...
      test("check_nft_ownership - success path with default network", async () => {
        ...
        (services.isNFTOwner as jest.Mock).mockImplementationOnce(
          (tokenAddress, ownerAddress, tokenId, network) => {
            return Promise.resolve(true);
          }
        );
      ...
      test("check_nft_ownership - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error checking NFT ownership: This is a string error"
        );
      });
      ...
      test("check_nft_ownership - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          params,
          services.isNFTOwner as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error checking NFT ownership: Test error"
        );
      });
      ...
      test("get_erc1155_token_uri - success path", async () => {
        ...
        (services.getERC1155TokenURI as jest.Mock).mockResolvedValue(
          "https://example.com/metadata/123" as never
        );
      ...
      test("get_erc1155_token_uri - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          params,
          services.getERC1155TokenURI as jest.Mock,
          mockError
        );

        verifyErrorResponse(
          response,
          "Error fetching ERC1155 token URI: Test error"
        );
      });
      ...
      test("get_erc1155_token_uri - success path with default network", async () => {
        ...
        expect(response).toHaveProperty("content");
        expect(response.content[0]).toHaveProperty("type", "text");
        expect(JSON.parse(response.content[0].text)).toHaveProperty(
          "uri",
          "https://example.com/metadata/123"
        );
      });
      ...
      test("get_erc1155_token_uri - error with non-Error object", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error fetching ERC1155 token URI: This is a string error"
        );
      });
    });
    ...
    describe("Contract Interaction Tools", () => {
      ...
      test("is_contract - error path", async () => {
        ...
        const response = await testToolError(
          tool,
          { address: mockTokenAddress, network: mockNetwork },
          services.isContract as jest.Mock,
          mockError
        );
    ...
    test("deploy_contract - error path", async () => {
      ...
      const response = await testToolError(
        tool,
        params,
        services.deployContract as jest.Mock,
        mockError
      );
    ...
    test("deploy_contract - error with non-Error object", async () => {
      ...
      expect(response).toHaveProperty("isError", true);
      expect(response.content[0].text).toContain(
        "Error deploying contract: This is a string error"
      );
    });
    ...
    describe("Documentation Search Tools", () => {
      ...
      test("search_docs - HTTP error response", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: Search failed: HTTP error! status: 404"
        );
      });
      ...
      test("search_docs - empty results", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: Search failed: No results found"
        );
      });
      ...
      test("search_docs - network error", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: Search failed: Network error"
        );
      });
      ...
      test("search_docs - unknown error type", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: Unknown error"
        );
      });
      ...
      test("search_docs - JSON parsing error", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: Search failed: Invalid JSON"
        );
      });
      ...
      test("search_docs - non-Error object thrown", async () => {
        ...
        expect(response).toHaveProperty("isError", true);
        expect(response.content[0].text).toContain(
          "Error searching docs: [object Object]"
        );
      });
