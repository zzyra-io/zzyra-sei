...
import { Download, Users, CreditCard, Activity, TrendingUp } from "lucide-react"
...
export default function AnalyticsPage() {
  ...
  const getTotalSignups = () => {
    if (!data) return 0
    return data.userSignups.reduce((sum: number, item: any) => sum + item.count, 0)
  }

  const getTotalRevenue = () => {
    if (!data) return 0
    return data.revenue.reduce((sum: number, item: any) => sum + item.amount, 0)
  }

  const getTotalExecutions = () => {
    if (!data) return 0
    return data.workflowExecutions.reduce((sum: number, item: any) => sum + item.count, 0)
  }

  const getAverageActiveUsers = () => {
    if (!data) return 0
    const total = data.activeUsers.reduce((sum: number, item: any) => sum + item.count, 0)
              ...
              {isLoading ? (
                ...
                <>
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{getTotalSignups()}</div>
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                      <CardTitle className="text-sm font-medium text-muted-foreground">Workflow Executions</CardTitle>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{getAverageActiveUsers()}</div>
                      <p className="text-xs text-muted-foreground mt-1">
