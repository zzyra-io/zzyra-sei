...
const installSessionKeyValidator = async (
  sessionKey: string,
...
export function useTransactionStatus(transactionId: string) {
  ...
  useEffect(() => {
    const ws = new WebSocket(
      `${WORKER_URL}/transactions/${transactionId}/status`
    );
...
const scheduleERC20Transfer = async (job: ERC20TransferJob) => {
  ...
  if (!validation.isValid) {
    throw new BadRequestException(
      `Transfer not authorized: ${validation.errors.join(", ")}`
    );
  }
...
export class ERC20TransferProcessor {
  ...
  async handleERC20Transfer(job: Job<ERC20TransferJob>) {
    ...
    try {
      ...
      if (balance < amountBase) {
        throw new Error(
          `Insufficient ${symbol} balance: ${formatUnits(balance, decimals)} available`
        );
      }

      // 6. Final policy validation (on-chain validator will also enforce)
      await this.validateTransactionPolicy(
        sessionKeyId,
        tokenAddress,
        recipient,
        amountBase
...
const validateERC20Transaction = async (params: {
  ...
  if (
    permission.recipientAllowlist?.length > 0 &&
