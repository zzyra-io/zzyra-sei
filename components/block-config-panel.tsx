  ...
  const renderConfigComponent = () => {
    ...
    switch (blockType) {
      case BlockType.PRICE_MONITOR:
        return <PriceMonitorConfig config={config} onChange={handleConfigChange} />
      ...
      case BlockType.TRANSACTION:
        return <TransactionConfig config={config} onChange={handleConfigChange} />
      case BlockType.GOAT_FINANCE:
        return <GoatFinanceConfig config={config} onChange={handleConfigChange} />
      default:
        ...
        return (
          <div className="p-4 text-center text-muted-foreground">
            <p className="mb-2">No specific configuration options for "{blockMetadata.label}" block type.</p>
            <p className="text-xs">You can still customize general settings in the General tab.</p>
          </div>
        )
    }
  }
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {["primary", "secondary", "blue", "green", "red", "yellow", "purple", "orange"].map((color) => (
                    <div
                      key={color}
                      className={`h-8 rounded-md cursor-pointer border-2 ${
                        (localNode.data.style?.accentColor || "primary") === color
                          ? "border-ring"
                          : "border-transparent"
                      }`}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleStyleChange({ width: Number.parseInt(e.target.value) })}
                  min={150}
                  max={400}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleStyleChange({ height: Number.parseInt(e.target.value) })}
                  min={100}
                  max={400}
                />
              </div>
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`p-2 rounded-md cursor-pointer text-center text-xs ${bg.value} ${
                        (localNode.data.style?.backgroundColor || "bg-card") === bg.value
                          ? "ring-2 ring-ring"
