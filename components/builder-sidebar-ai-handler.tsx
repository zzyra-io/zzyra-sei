...
import type { CustomBlockDefinition, BlockParameter } from "@/types/custom-block";
...
const generateLogicFromInputsOutputs = (
  ...
  const logicStatements = inputs.map((input, index) => {
    // Map to the right output if exists, otherwise use the first output
    const output = index < outputs.length 
      ? outputs[index] 
      : outputs.length > 0 ? outputs[0] : { name: "result", type: "string" };
...
export const parseInputsFromString = (inputStr: string) => {
  if (!inputStr || inputStr.trim() === "") {
    return [
      { name: "input1", type: "string", description: "Default input" }
    ];
  }
  ...
  return inputLines.map((line, index) => {
    ...
    if (name.toLowerCase().includes("count") || 
      ...
      type = "number";
    } else if (name.toLowerCase().includes("enabled") || 
...
export const validateAiBlockForm = (aiBlockForm: {
  ...
  if (aiBlockForm.blockDescription.trim().length < 10) {
    return { valid: false, error: "Block description must be at least 10 characters" };
  }
  
  if (aiBlockForm.blockDescription.trim().length > 200) {
    return { valid: false, error: "Block description must not exceed 200 characters" };
  }
  ...
  try {
    ...
    for (const input of inputs) {
      ...
      if (!input.type) {
        return { valid: false, error: `Input '${input.name}' must have a type` };
      }
    }
  } catch (error) {
    return { 
  ...
  if (aiBlockForm.blockOutputs.trim()) {
    try {
      ...
      for (const output of outputs) {
        ...
        if (!output.type) {
          return { valid: false, error: `Output '${output.name}' must have a type` };
        }
      }
    } catch (error) {
      return { 
