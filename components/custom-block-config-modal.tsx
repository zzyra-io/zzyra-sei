...
export function CustomBlockConfigModal({
  ...
  const handleSave = async (config: { customBlockId: string; inputs: Record<string, BlockInputValue> }) => {
    try {
      setLoading(true);
      ...
      toast({
        ...
        description: 'There was an error saving the custom block configuration.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
